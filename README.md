...
**這是 2.0 版 repo**（前身 `Three-Kingdoms-Biographies` 1.0 版保留不動、
單獨存放，不再更新）。2.0 版最主要的變動是首頁跟人物總覽的整個重新設計，
資料 schema（`data/characters/*.json` 的結構）跟 1.0 完全相容、沒有改動。
...
- **奈奈（ChatGPT）**：負責查證史料、整理人物資料，輸出成本專案的 JSON
  schema 格式；有時也會被拿來做視覺／前端試作稿
- **Claude（你）**：負責架構設計、資料整合、程式碼撰寫、跑測試驗證，以及
  在奈奈的查證或試作稿出現疏漏時提出質疑、必要時自己動手查證核對

**你的角色不是被動套資料或照單全收試作稿**。這個專案一路走來，好幾次
重要的修正都是 Claude 主動發現問題後跟使用者提出來的，包括但不限於：

- 奈奈曾誤把「劉禪生母」的「母」字誤判成親屬排序的父母輩
- 曾經有一段「日語諧音」考證查無實據卻被直接寫進資料
- 「隆中對」原本被誤放進著作清單
- 奈奈曾經交付一份「首頁試作稿」，實際上是用某個網站產生工具做的
  Next.js + Cloudflare Workers + D1 資料庫 + 「用 ChatGPT 登入」的完整
  應用程式骨架，跟本專案「純 HTML/CSS/JS、無框架無後端」的立命之本直接
  衝突；且該試作稿把人物資料整個重複造了一份、跟 `data/` 脫鉤。這次是
  在動手套用前，透過檢查檔案結構抓到的問題

收到奈奈整理的資料或試作稿後，先做交叉驗證，抓到問題要老實跟使用者說，
不要為了效率而略過查證，也不要因為東西「看起來很完整」就假設它跟本專案
的技術路線相容。
...
字號、頭像路徑。目的是先讓人物卡片跟連結佔位，之後陸續補完整內容。**接手
時看到某個人物頁面幾乎全空，不代表資料出錯，先檢查 `dataStatus` 欄位是否
存在**——完整資料一律會有 `"dataStatus": "reviewed-draft"`，空檔人物則
完全沒有這個欄位。
...
```
tw3k/
├── index.html               卷首（入口頁）：說書人主視覺 + 五個勢力入口
│                             卡片 + 全站快速搜尋，不列出人物本身
├── faction.html              單一勢力的人物總覽頁「模板」，五個陣營共用
│                             同一份，靠網址 ?faction=wei 這種參數決定內容
...
│   ├── portal.js               卷首（index.html）邏輯：五個勢力入口卡片
│   │                          渲染（人數即時算，不寫死）、全站快速搜尋
│   ├── faction-portal-meta.js  五個勢力入口的「敘事層」中繼資料：君主
│   │                          姓名、七言對句格言、代表肖像圖路徑、浮水印
│   │                          用的單字。不放人物清單或人數——人數永遠從
│   │                          data/index.json 即時算，避免跟正式資料脫鉤
│   ├── listing.js              faction.html 邏輯：讀網址上的 ?faction=
│   │                          參數、渲染勢力橫幅、渲染竹簡人物列表、
│   │                          站內搜尋（只在該陣營內搜，沒有勢力篩選
│   │                          pills——首頁的入口卡片已經負責選陣營了，
│   │                          這裡不重複做）
│   └── character.js            詳細頁邏輯（含親屬排序、雙層兄弟關係
│                                渲染、自我引用頭像判斷、返回連結邏輯
│                                等進階邏輯）
├── data/
│   ├── index.json              全站人物的輕量索引（含 published 欄位）
│   ├── sources.json            共用來源目錄
...
└── images/
    ...
    │                            人物詳細頁與竹簡列表共用同一張
    ├── {name}_main.png           五個陣營君主的「去背」肖像（透明背景
    │                            PNG），只用在首頁入口卡片跟勢力橫幅，
    │                            跟上面人物自己的頭像是不同用途、不同圖，
    │                            不要混用（例如劉備、孫權身兼「人物」跟
    │                            「君主」雙重身分，會各自有一張頭像圖
    │                            跟一張君主去背圖，兩張不必是同一張）
    ├── storyteller-cutout.png    卷首說書人的去背立繪（透明背景）
    ├── ink-wash-bg.jpg           卷首背景的水墨遠山圖，跟說書人去背圖
    │                            是分開的兩層，各自可以獨立替換
...
**沒有 `js/main.js`**——這是 1.0 版首頁（純卡片牆）的邏輯檔案，2.0 版
首頁改版後已經被 `portal.js` + `listing.js` 取代，如果你在這個 repo 裡
還看到 `main.js`，那是沒清乾淨的殘留，可以直接刪，沒有任何 HTML 引用它。

## 首頁與勢力總覽頁架構（2.0 版新增，重點）

### 為什麼拆成 `index.html` + `faction.html` 兩個頁面

1.0 版首頁是單一頁面的人物卡片牆（全部人物 + 勢力篩選 pills）。2.0 版
拆成兩層：

- **`index.html`（卷首）**：不列出人物本身，只做「氣氛入口」——說書人
  立繪、五個勢力入口卡片（各自的陣營色調、君主去背肖像、大字浮水印、
  即時人數），跟一個全站快速搜尋（打字直接列出符合的人物，點了直接跳
  `character.html`）
- **`faction.html`（勢力總覽）**：使用者從卷首點某個勢力入口進來後看到
  的頁面，一次只服務**一個**陣營（網址一定要帶 `?faction=wei` 這種
  參數），裡面是那個陣營專屬色調的橫幅 + 站內搜尋 + 竹簡風格人物列表

**這個頁面刻意沒有「勢力篩選 pills」跟「全部人物」模式**——因為首頁的
五個入口卡片已經負責「選陣營」這件事了，`faction.html` 裡再放一次篩選
按鈕會是功能重複。網址沒帶 `faction` 參數、或帶了不存在的陣營 id，
`listing.js` 會直接 `location.replace("index.html")` 導回卷首，不會顯示
空白或錯誤頁面。

### 五個勢力入口／陣營色調系統

`js/faction-portal-meta.js` 定義了五個陣營（`wei`／`shu`／`wu`／
`donghan`／`qunxiong`，對應 `factions.json` 的 `filterGroup` id）各自的
君主姓名、格言、浮水印單字、去背肖像路徑。**這五個陣營固定顯示**，就算
`donghan`／`qunxiong` 目前完全沒有已發布人物，也會顯示卡片跟「尚無收錄
人物」的字樣，不會因為沒人就不顯示入口——因為之後一定會補人進去。

**陣營色調**（`--tone-wei`／`--tone-shu`／`--tone-wu`／`--tone-han`／
`--tone-qun`，定義在 `style.css` 的 `:root`）不只用在首頁入口卡片，
`faction.html` 整頁（頁面底色的淡色暈、橫幅、竹簡列表每一列的背景漸層、
邊框）都會依網址帶的陣營 id 切換對應色調，靠 `body` 的
`.faction-page--wei` 這類 modifier class 掛 `--tone` 這個 CSS 自訂屬性，
下面的元素靠 `color-mix(in srgb, var(--tone) X%, ...)` 各自決定要混多少
比例。**如果之後又覺得色調不夠分明，先檢查是不是又把某個漸層的終點寫死
...
這樣浮水印只會在肖像沒蓋到的縫隙／角落露出來，不會疊在君主臉上。這是
試過好幾種方案後定案的：疊在肖像上面（`z-index` 比肖像高）會讓筆畫直接
畫過五官，看起來雜亂；用 `mix-blend-mode` 疊在肖像上面雖然比較融合，
但使用者實測後還是覺得不夠乾淨。**不要為了「讓浮水印更明顯」就把
`z-index` 調到比肖像高**，這件事已經來回調過三次，目前的方案是使用者
確認滿意的版本。

### 君主去背肖像 vs 人物自己的頭像

`faction-portal-meta.js` 裡每個陣營的 `portrait` 欄位指向的是「君主的
去背肖像」（例如 `images/cao_cao_main.png`），這跟人物自己在
`data/index.json`／`data/characters/{id}.json` 裡的 `avatar` 欄位
（例如 `images/cao_cao.png`，如果曹操之後真的建檔的話）是兩個獨立系統，
不要混用、也不要假設兩者是同一張圖。**理由**：君主入口卡片需要的是「去背、
半身或立姿、可以裁到卡片邊緣」的肖像，人物頭像則是「方形、可以裁成圓形
頭像框」的近拍照，兩種構圖需求不一樣，之後想幫某個陣營的君主換圖，只要
改 `faction-portal-meta.js` 裡的 `portrait` 路徑，不會牽動到人物自己的
資料或頭像。

某個陣營暫時沒有去背肖像時，`portrait` 留 `null`，畫面會顯示「君主圖
預留」的虛線佔位框，不會顯示破圖或空白。

### 竹簡人物列表

`faction.html` 的人物清單是「橫條堆疊」設計（`.slip`），不是卡片格：
頭像＋姓名＋字號常駐顯示，滑鼠移過去（`.slip:hover`）才展開陣營標籤
（「史實記載」／「後世文學」）跟「入卷 →」連結。這兩個標籤**不是寫死的
資料**，是 `listing.js` 的 `loadPersonTags()` 讀該人物自己 JSON 裡的
`historicalBio`／`romanceBio` 陣列是否真的有內容來即時判斷——空殼佔位
人物（法正、關平、關興這種）兩個標籤都不會顯示，這是正確行為，不是
bug。

**滾動時 hover 狀態會跟著游標走**（模擬「手指停在原地、書一直滑過去」
的手感）：這是刻意額外寫的邏輯，因為瀏覽器原生的 `:hover` 只有滑鼠真的
移動時才會重新判定，單純滾動（滾輪）不會觸發，所以 `initSlipHoverFollow()`
額外用 `elementFromPoint()` 在滾動時手動補一個 `.is-hovered` class。
**如果之後要改這段邏輯，注意兩件事**：(1) 滑鼠真的移動時要清掉
`.is-hovered`、讓原生 `:hover` 接管，不然會有「捲到某一列後 hover 卡住
不放」的殘留 bug；(2) `.slip` 元素一定要有 `flex-shrink: 0`，不然
flexbox 會把每一列壓扁塞進固定高度的容器，而不是讓清單真的超出去產生
可以捲動的內容。

手機版目前把標籤跟「入卷」連結改成常駐顯示、不做 hover 展開（觸控裝置
沒有真正的 hover，藏起來反而永遠看不到）。
...
底下「測試方式」那節的方法確認 0 個缺漏，這是最基本的驗收標準。**
...
- `filterGroups`：粗分類——東漢／群雄／曹魏／蜀漢／孫吳／西晉，跟
  `faction-portal-meta.js` 裡的五個入口一一對應（`jin` 西晉目前沒有
  入口卡片，姜維是唯一跟西晉沾邊的人物，但他終其一生效忠蜀漢，不會被
  歸進西晉分類）
...
**`faction.html` 只計算已發布（`published: true`）人物**：`listing.js`
裡的人數統計、竹簡列表都會過濾掉未發布人物。
...
- 所有人物頭像一律用 `avatar-ring`（人物詳細頁頁首大頭像）或
  `avatar-ring--sm`（勢力歷程、親屬列表）包裝，套用一致的金色漸層框，
  不要有裸的 `<img>` 標籤散落各處；竹簡列表用的是另一組獨立的
  `.slip-avatar`（見上方「竹簡人物列表」一節），不是同一套 class，兩者
  外觀刻意不同（金框 vs 素框），不要混用或以為是同一個元件
...
**人物詳細頁的「返回人物列表」連結是動態算出來的**，不是寫死
`index.html` 或 `faction.html?faction=all`（後者現在已經不存在了）：
`character.js` 的 `resolveBackLinkUrl()` 優先讀 `document.referrer`，
如果是從同網域的 `faction.html` 進來的，就記住原本那個網址（含
`?faction=` 參數），讓使用者點返回會回到他原本瀏覽的那個陣營；抓不到
referrer 時，退回這位人物自己所屬陣營的 `faction.html?faction=xxx`；
再抓不到（例如「找不到人物」錯誤頁，不知道是哪個陣營）才退回卷首
`index.html`。
...
JSON 會每次重新抓（也因此人物自己的 JSON 故意不用這個快取函式，見
`loadJson()` 上的註解）。
...
人物的 `personId` 就算填了也不會顯示成連結，首頁快速搜尋跟該陣營的竹簡
列表也都不會列出他。
...
**如果是請奈奈做視覺／前端試作稿**（不是查證人物資料），提示詞要明確
框住技術路線：純 HTML/CSS/原生 JS，不要用任何框架、建置工具、後端或
資料庫；如果她主動說要用某個「網站產生工具」，先確認產出的到底是什麼
（見本文件最前面「專案分工」一節提到的踩雷案例），拿到試作稿後先檢查
檔案結構跟 `package.json` 存不存在，再決定要不要套用。

## 測試方式（每次整合完新資料或改完前端都要跑）
...
5. **實際渲染測試**：用 `npm install jsdom`，寫一段腳本模擬
   `window.fetch` 讀本機檔案，分別載入 `index.html`／
   `faction.html?faction=xxx`（五個陣營都要測，含空陣營）／
   `character.html?id=xxx`（全部已發布人物都要測），確認：
   - 卷首五個入口卡片人數正確、連結網址正確
   - `faction.html` 網址缺參數或帶不存在的陣營 id 時會導回 `index.html`
   - 每個人物頁 `.source-chip--missing` 元素數量為 0
   - 親屬／勢力歷程渲染數量符合預期、連結（`<a>` 標籤）正確生成
   - 頁籤點擊不拋出例外
6. **CSS／JS 改動後，額外用瀏覽器截圖確認視覺**：jsdom 不會真的渲染
   CSS，只能驗證 DOM 結構跟 class 是否正確套用，改動視覺相關的東西
   （尤其是顏色、遮罩、z-index 疊層）務必額外截圖肉眼確認，本文件
   「首頁與勢力總覽頁架構」一節提到的好幾個 bug 都是純看程式碼看不出來、
   一定要實際渲染才會發現的
7. 測試完成後才交付，並清掉 `node_modules`／`package.json`／暫存資料夾
   ／截圖檔，不要把這些留在交付的專案裡

### 卷首引言文字

`.storyteller-copy-overlay`／`.storyteller-quote`／`.storyteller-caption`
現在是啟用狀態，顯示說書人的開場白（目前是「亂世烽煙分鼎足，一卷山河記
群英。」＋「卷首引路人・無名說書客」）。**這段文字的寬度／字級是刻意收
窄配小的**：`.storyteller-quote` 設了 `white-space: nowrap`（七字一句
強制不換行，寧可字小也不要斷句斷得難看），`.storyteller-copy-overlay`
的 `max-width` 也收到 `min(300px, 40%)`——這是跟前面「浮水印不要疊到
君主臉」同一種教訓：文字區塊一開始比較寬（`330px`），會讓字句尾端剛好
畫過說書人的眉毛／臉頰，後來才收窄跟縮小字級解決。**之後如果想換成更長
的句子，要重新確認寬度會不會又蓋到他的臉**，不要只看程式碼、一定要截圖
肉眼確認。

## 之後可能想做的事

- **東漢、群雄兩個陣營目前完全沒有已發布人物**，之後要補人的話，判準
  已經在跟使用者的討論中定案：東漢＝還在漢廷體制內做事、聽命於皇帝或
  大將軍調度的人（例如盧植、何進、朱儁）；群雄＝已經形成自己武裝、
  割據一方、只是名義上奉漢的人（例如董卓、呂布、袁紹、袁術）
- 曹魏、東漢、群雄三個陣營的君主去背肖像（`cao_cao_main.png`／
  `liu_xie_main.png` 已經有了，`qunxiong` 目前還沒有，因為群雄沒有
  單一君主，`ruler` 欄位刻意留空，要不要放一張代表性人物的圖還沒定案）
...
- 目前的「空檔」人物（法正、關平、關興、孫權）陸續要補完整資料

...
**浮水印大字的字型：字型檔留著，但目前刻意停用**。原本用 Google
Fonts 的 Ma Shan Zheng，但那款字型的繁體字集不完整——「漢」「吳」這
兩個字完全沒被收錄（作者主要照簡體字集做的，這兩個字剛好簡繁不同碼
位），瀏覽器會自動退回備用字型，導致有些陣營的浮水印看起來跟其他陣營
風格不一樣。為此自己 host 了「莫大毛筆字體」（Bakudai）解決這個缺字
問題，這是專門為了補足繁體中文缺字問題而做的開源字型（來源：
...
人物姓氏用字，共 20KB，原始檔案是 6.5MB）。

**但實測比對後，使用者覺得備援字體（Noto Serif TC 粗體）看起來反而更
好看**，所以現在 `--font-brush` 刻意不包含 `"Bakudai"`，`@font-face`
宣告也整個註解掉，改用 Noto Serif TC 粗體。**這不是又忘記接字型、是
比較過兩種效果後的刻意選擇**，`css/style.css` 裡對應位置有寫這個決定
的來龍去脈。字型檔跟 `@font-face` 宣告都還留在專案裡沒刪，想重新啟用
只要取消註解、把 `--font-brush` 改回加上 `"Bakudai"` 就好，不用重新
子集化。**之後如果要用到子集裡沒有的字**（萬一真的重新啟用），不能
直接在 CSS 裡多打字了事，要回頭用 fonttools 的 `pyftsubset` 重新從
`bakudaifont` 原始 repo 子集化，加進新字後重新產生
`bakudai-watermark.woff2`。
